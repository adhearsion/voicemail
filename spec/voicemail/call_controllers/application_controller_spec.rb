  ...
  describe "#main_menu" do
    context "with the defaults" do
      it "passes to MainMenuController" do
        subject.should_receive(:pass).once.with Voicemail::MailboxMainMenuController, include(mailbox: mailbox[:id], storage: storage_instance)
    ...
    context "with a custom class" do
      ...
      it "passes to custom controller class" do
        subject.should_receive(:pass).once.with Foo, include(mailbox: mailbox[:id], storage: storage_instance)
        controller.main_menu
      end
    end
  end
